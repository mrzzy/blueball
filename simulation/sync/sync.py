...
def server_step_input(
        ...
        update: Callable[[State,Input],State]):
...
def server_step(ty: TimedState, txs: Iterable[TimedInput], update):

    mem_txs = make_timeline()
    mem_tys = make_timeline([ty])

    for tx in txs:
        yield server_step_input(tx, mem_txs, mem_tys, update)
